"""
evalution module for test datasets
"""
...
class Evaluator:
    ...
    def evaluate(self, test_loader: DataLoader, threshold: Optional[float] = None) -> Dict[str, float]:
        ...
        args:
            ...
            threshold: optional probability threshold for classfying fake 
        ...
        with torch.no_grad():
            for images, labels in test_loader:
                ...
                if hasattr(outputs, "logits"):   # huggingface automodel case, if it gives us logits
                    outputs = outputs.logits
                probabilities = torch.softmax(outputs, dim=1) # convert logits into probabilities from 0-1
            
                # apply threshold if provided
                if threshold is not None:
                    predictions = (probabilities[:, 1] >= threshold).long() # checks if every fake probablity exceeds threshold
                else:
                    _, predictions = outputs.max(1) # goes back to raw logits, we dont care about score just verdict
    ...
    def collect_probs_and_labels(self, loader: DataLoader) -> Tuple[np.ndarray, np.ndarray]:
        """collect probabilities for fake class and labels from loader"""
        probs = []
        labels = []
        with torch.no_grad(): # dont need gradients bc not training
            for images, y in loader:
                images = to_device(images, self.device)
                outputs = self.model(images)
                if hasattr(outputs, "logits"):
                    outputs = outputs.logits
                p = torch.softmax(outputs, dim=1)[:, 1] # grabs fake probability only
    ...
    def find_optimal_threshold(self, loader: DataLoader, strategy: str = "youden") -> Dict[str, float]:
        """
        find optimal prob threshold on a validation set, strategy is "youden" (maximize TPR-FPR) or "f1" (maximize f1)
        """
        ...
        for t in thresholds:
            ...
            if strategy == 'f1':
                ...
                fpr, tpr, _ = roc_curve(labels, probs >= t) # tpr = # fakes caught, fpr = # reals called fake, we want to max (tpr - fpr)
    ...
    def load_optimal_threshold(self, json_path: str) -> float:
        # falls back to 0.5 if file missing/invalid
        try:
