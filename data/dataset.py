"""
dataset classes, handles loading + preprocessing of real/fake images
"""
...
class FSDNetDataset(Dataset):
    ...
    def __init__(self, 
                 ...
                 split: str = "train"):
        """
        required args to initialize dataset:
            real_dir: directory containing real images
            fake_dir: directory containing fake images
            image_size: size to resize images to (224 by default)
        ...
        # optional split subdirectories: real_dir/{train,val,test}, fake_dir/{train,val,test}
        ...
        fake_src = fake_dir_split if os.path.isdir(fake_dir_split) else fake_dir # get final dirs
    ...
    def _get_image_paths(self, directory: str) -> List[str]:
        """get list of image file paths from dir"""
        if not os.path.isdir(directory):
            return []
        valid_exts = {".jpg", ".jpeg", ".png", ".bmp", ".webp"}
        paths = []
        for root, _, files in os.walk(directory):  # recursively visits all image files
            for f in files:
    ...
    def _get_default_transforms(self) -> transforms.Compose:
        return transforms.Compose([
            ...
            transforms.ToTensor(),
            transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225]) # all this is doing is turning images into tensors that GPU can deal with
        ])
    ...
    def __getitem__(self, idx: int):
        ...
        return image, label # in our case returns pytorch tensor and real/fake

class CombinedImageVideoDataset(Dataset):
    """
    combined dataset for images and video extracted frames
    expected directory layout:
      - image reals/fakes from provided dirs 
...
def _stratified_split_indices(labels: List[int], splits: Tuple[float, float, float]) -> Tuple[List[int], List[int], List[int]]:
    ...
    def split_group(group: List[int], ratios: Tuple[float, float, float]):
        random.shuffle(group)  # randomizing order
        n = len(group)
    ...
    train_f, val_f, test_f = split_group(fake_idxs, splits) # preserves real/fake ratios
    train = train_r + train_f
    val = val_r + val_f
    test = test_r + test_f
    random.shuffle(train); random.shuffle(val); random.shuffle(test) # randomization pt. 2
    return train, val, test


def create_combined_data_loaders(config) -> Tuple[DataLoader, DataLoader, DataLoader]:
    """
    create data loaders that combine images + optional video frames with 70/15/15 split
    applied across combined pool when explicit split dirs are absent
    """
    ...
    # if both images and frames already split into subfolders, build loaders directly
    has_explicit_splits = (
        ...
        (not frames_dir or os.path.isdir(os.path.join(frames_dir, 'train'))) # either we arent using video or video frames path exists
    )
    ...
    if has_explicit_splits:
        ...
        # build single combined pool and split stratified
    ...
    def make_loader(ds: Dataset, shuffle: bool) -> DataLoader:
        if getattr(config.training, 'sampler_balance', False) and shuffle:
            ...
            if hasattr(ds, 'labels'):
                ...
                labels_np = np.zeros(len(ds), dtype=np.int64) # just assume everything is real
            class_sample_count = np.bincount(labels_np, minlength=2)
            class_weights = 1.0 / np.maximum(class_sample_count, 1)
            sample_weights = class_weights[labels_np] 
            sampler = WeightedRandomSampler(weights=torch.DoubleTensor(sample_weights), num_samples=len(ds), replacement=True) # allow replacement for class balancing
            return DataLoader(
                ds,
